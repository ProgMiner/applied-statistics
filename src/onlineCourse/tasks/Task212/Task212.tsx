...
export class Task212 extends Task<Task212State> {
    ...
    private static checkRow(row: string[]) {
        return row.filter(verifyNumber).length === row.length;
    }

    protected checkParameters(): boolean {
        const { e, n, changeFirst, changeSecond } = this.state;

        return Task212.checkRow(e) && Task212.checkRow(changeFirst)
            && Task212.checkRow(changeSecond) && Task212.checkRow(n);
    }
        ...
        return (
            <>
                <div className="p-inputgroup half-margin-bottom">
                    ...
                    <span className="p-inputgroup-addon">
                        <ValidationIcon valid={Task212.checkRow(e)} />
                    </span>
                </div>
                ...
                <div className="p-inputgroup half-margin-bottom">
                    ...
                    <span className="p-inputgroup-addon">
                        <ValidationIcon valid={Task212.checkRow(n)} />
                    </span>
                </div>
                ...
                <div className="p-inputgroup half-margin-bottom">
                    ...
                    {[0, 1, 2, 3].map(i => (
                        <React.Fragment key={i}>
                            ...
                            <InputText type="number" value={changeFirst[i]}
                                       onChange={this.onRowChange('changeFirst', i)} />
                        </React.Fragment>
                    ))}

                    <span className="p-inputgroup-addon">
                        <ValidationIcon valid={Task212.checkRow(changeFirst)} />
                    </span>
                </div>
                ...
                <div className="p-inputgroup half-margin-bottom">
                    ...
                    {[0, 1, 2, 3].map(i => (
                        <React.Fragment key={i}>
                            ...
                            <InputText type="number" value={changeSecond[i]}
                                       onChange={this.onRowChange('changeSecond', i)} />
                        </React.Fragment>
                    ))}

                    <span className="p-inputgroup-addon">
                        <ValidationIcon valid={Task212.checkRow(changeSecond)} />
                    </span>
                </div>
            </>
        );
